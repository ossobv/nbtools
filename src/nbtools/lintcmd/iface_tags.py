...
# For now, we can only tell if this makes sense if two dcim.interface types
# are directly connected. For FEC-OFF that probably also makes sense.
# For CORELINK and EBGP we would likely want the end-to-end to match.
...
class InterfaceTagsCommand(LintCommand):
    ...
    def find(self):
        ...
        for tag in self._tags:
            # One request per tag: /dcim/interfaces/?tag=corelink.
            # Asking for all three at once would come back as a union
            # with no way to tell which interface was in it for which
            # tag.
