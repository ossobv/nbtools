...
def get_clusters(nbapi, name):
    ...
    return list(nbapi.virtualization.clusters.filter(name__ie=name))
...
class DiscoveredItemsCommand(LintCommand):
    ...
    help = (
        ...
        f'under the "{DISCOVERY_CLUSTER}" cluster. Nothing here is broken '
        'as such: they are the things nobody has placed by hand yet, '
        'which is what makes them worth a periodic look.')
